...
outside_BS = 5;
outside_BS = round(deg2pix_YR(outside_BS));
...
%        then do another flip to clear this half a sec later
       vbl = Screen('Flip', window, vbl + (0.5/ifi - 0.5) * ifi);
%       while ourtime < 0.5
% %         Screen('SelectStereoDrawBuffer', window, 1);   %RIGHT
% %         Screen('CopyWindow', rightFixWin, window, [], rightScreenRect);
% %         Screen('SelectStereoDrawBuffer', window, 0);   %LEFT
...
%   Screen('CopyWindow', rightFixWin, window, [], rightScreenRect);
%   Screen('SelectStereoDrawBuffer', window, 0);   %LEFT
%   Screen('CopyWindow', leftFixWin, window, [], wind   owRect);
...
if keyIsDown
  if keyCode(escapeKey); resp2Bmade = false; endtime = GetSecs; sca;
...
%             [keyIsDown,secs, keyCode] = KbCheck;%
%             if keyCode(escapeKey); resp2Bmade = false; sca; 
%             elseif keyCode(leftKey); resp2Bmade = false; curr_response = 1;
%             elseif keyCode(rightKey); resp2Bmade = false; curr_response = 2;
%             end
%       end %while
  end %end if
