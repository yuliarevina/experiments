...
filename = sprintf('PolAng_%s_%s_%s_%s.mat', todaydate, subCode, num2str(subAge), subGender);
...
%     3x3-geocaching-logo-sticker_500_1.jpg
    theImageLocation = sprintf('%sPolar_Wedge_%d.png', dir,imcount);
    % load the image from location
    [img, ~, alpha] = imread(theImageLocation);
%     img = rgb2gray(img); %take info only from red channel, we don't need color info
...
for i = 1:12 %12 repetitions of the cycle
    ...
    for rots = 1:nrotations
        ...
        Screen('DrawTexture', window, texture2(imnumber(1)), [], texturerectangle, rotation, 0);
