...
%basic rect of stimulus
stim_rect = [0 0 bar_width bar_length]; %width 1.73 deg = 37 px (on station 2 CRT)
stim_rect_centre = CenterRectOnPointd(stim_rect,BS_center_h2,BS_center_v); 

% grey occluder for stimulus localizer 
occluder_stim_rect = [0 0 bar_width bar_length]; %width 1.73 deg = 37 px (on station 2 CRT)
occluder_stim_rect_centre = CenterRectOnPointd(occluder_stim_rect,BS_center_h2,BS_center_v+deg2pix_YR_MRI(4)); %basically present the bar but 4deg lower which will reveal the top 4deg of the stim
% 
...
% 3. Stimulus Both
% 4. Fixation (both eyes)

% e.g.
% Fix | Stim 1 | Fix | Stim 3 | Fix | Stim 2 | Fix | Stim 4

%all possible combinations
allstimscomboslocalizer = perms(1:3);

% we need to choose 6 out of all poss ones
to_use = randperm(6,6);
to_use = [to_use, randperm(6,4)]
...
% our aperture:
% we need three target textures as we have target in 2 conditions
...
% STIMULUS
% new for follow up condition 3
% OPEN OFF SCREEN WINDOW
aperture(3)=Screen('OpenOffscreenwindow', window, grey);

% MAKE TRANSPARENT RECT THE SIZE OF THE STIM
% First we clear out the alpha channel of the aperture disk to zero -
% In this area the noise stimulus will shine through:
Screen('FillRect', aperture(3), [1 1 1 0], stim_rect_centre);
% Put on a grey occluder to keep only the top part of the checkerboard
Screen('FillRect', aperture(3), [grey grey grey 1], occluder_stim_rect_centre);
% % SURROUND
% % condition 4
% 
% % OPEN OFF SCREEN WINDOW
