...
if strcmp(bs_eye, 'left')
    ...
    DrawFormattedText(window, 'This is the location of blindspot \n \n Check you cannot see it \n \n by closing your LEFT eye and fixating on the + \n \n Waiting for Experimenter Key press...', 'center', 'center', textcolor, [],[]);
end
...
%occluder rect for the eye of origin control
% we'll place an occluding rectangle on the stimulus to only show the top
% 4deg of the bar. We could also resize the bar and work out the
% coordinates this needs to be presented at but placing an overall occluder
% is probably the easiest way to do it here

occluder_for_eye_of_origin_control = [0 0 bar_width bar_length]; %width 1.73 deg = 37 px (on station 2 CRT)
occluder_for_eye_of_origin_control_centre = CenterRectOnPointd(occluder_for_eye_of_origin_control,BS_center_h2,BS_center_v+deg2pix_YR_MRI(4)); %basically present the bar but 4deg lower which will reveal the top 4deg of the stim
% 
for i = 1:5
...
%                 disp('normal')
        ...
        switch runseq(block,stim,thisrun)
            ...
            case 3
                ...
                messagenexttrial = 'Next: Deleted Sharp'; %now Del Sharp
                disp(messagenexttrial);
            case 4
                %messagenexttrial = 'Next: Deleted Sharp';
                messagenexttrial = 'Next: Control Fellow Eye'; %now Control bar segment presented in Fellow eye for eye of origin analysis
                disp(messagenexttrial);
            case 5
%               messagenexttrial = 'Next: Deleted Fuzzy';
                messagenexttrial = 'Next: Control BS Eye'; %now Control bar segment presented in BS eye for eye of origin analysis
                whicheye = 'BS';
                disp(messagenexttrial);
            case 6
...
%           Screen('DrawTexture', window, gratingtex(2,1), srcRect, dstRectStim_BS_r); %gratingtex(i,1) high amplitude ie high contrast
            ...
            switch runseq(block,stim,thisrun)
                ...
                case 2 %Blindspot - special case, we need to present to the other eye!
                    ...
                    %
                    %we don't need to do anything else
                case 3 %now it's del Sharp (was %Occluded)
                    if stereoModeOn
                        Screen('SelectStereoDrawBuffer', window, 1);  %RIGHT
                    end
                    Screen('DrawTexture', window, gratingtex(2,1), srcRect, dstRectStim_BS_r); %gratingtex(i,1) high amplitude ie high contrast
...
%                         Screen('FrameOval', window, [(white*0.5*brightness)/2], occluderRectCentre_Expt, 3);
%                     end
                case 4 % now Control Fellow Eye (was %Deleted sharp)
                    if stereoModeOn
                        Screen('SelectStereoDrawBuffer', window, 1);  %RIGHT
                    end
                    Screen('DrawTexture', window, gratingtex(2,1), srcRect, dstRectStim_BS_r); %gratingtex(i,1) high amplitude ie high contrast
%                     %grey mask
                    Screen('FillRect', window, grey_bkg, occluder_for_eye_of_origin_control_centre); %grey occluder
...
%                         Screen('FillOval', window, grey_bkg, occluderRectCentre_Expt, maxDiameter); %grey occluder
%                     end
                case 5 %now Control BS Eye (was %Deleted fuzzy)
                    if stereoModeOn
                        Screen('SelectStereoDrawBuffer', window, 1);  %RIGHT
                    end
                    Screen('DrawTexture', window, gratingtex(2,1), srcRect, dstRectStim_BS_r); %gratingtex(i,1) high amplitude ie high contrast
