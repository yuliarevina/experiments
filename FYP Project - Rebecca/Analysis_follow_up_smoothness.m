...
%% figure
figure; plot(1:7, results(1:7,1,1), 'ro-') %BS
hold on;
plot(1:7, results(1:7,1,2), 'bs-') %Fellow
plot(1:7, results(1:7,1,3), 'go-') %Fellow annulus
...
%             disp('Deleted Fuzzy')
    ...
    OutOfNum = [ntrialseachcond ntrialseachcond ntrialseachcond ntrialseachcond ntrialseachcond ntrialseachcond ntrialseachcond];
...
BS_confidence = nan(ntrialseachcond,7) ;
Fellow_confidence = nan(ntrialseachcond,7) ;
Annulus_confidence = nan(ntrialseachcond,7) ;

BS_correct_confidence = nan(ntrialseachcond,7) ;
BS_incorrect_confidence = nan(ntrialseachcond,7) ;

Fellow_correct_confidence = nan(ntrialseachcond,7) ;
Fellow_incorrect_confidence = nan(ntrialseachcond,7) ;

Annulus_correct_confidence = nan(ntrialseachcond,7) ;
Annulus_incorrect_confidence = nan(ntrialseachcond,7) ;
...
for i = 1:3; %conditions
    ...
    for j = 1:7 %Orientations
        ...
        % renumber conditions to what they are in the subjectdata matrix
            ...
            if j == 1 || j == 2 || j == 3 %if the comparison orientation was below 45, then we need all trials where they answered 'standard' for correct answer and 'comparison' for incorrect
                %---------------------------------------------------------------------
            ...
            elseif j == 5 || j == 6 || j == 7 %if the comparison orientation was above 45, then we need all trials where they answered 'comparison'
                %--------------------------------------------------------------------
