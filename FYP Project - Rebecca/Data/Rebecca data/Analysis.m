...
meanBSPSE = mean(PSE_Slope_BS(subjectsBS,1));
meanFellowPSE = mean(PSE_Slope_Fellow(subjectsFellow,1));


%medians
medianBSPSE = median(PSE_Slope_BS(subjectsBS,1));
medianFellowPSE = median(PSE_Slope_Fellow(subjectsFellow,1));

%group data for boxplot
Alldatatoplot = [PSE_Slope_BS(subjectsBS,1); PSE_Slope_Fellow(subjectsFellow,1)];
groups = [ones(1,length(subjectsBS))';2*ones(1,length(subjectsFellow))'];

individdata = {PSE_Slope_BS(subjectsBS,1), PSE_Slope_Fellow(subjectsFellow,1)};
...
[p,h,stats] = signrank(PSE_Slope_BS(intersect(subjectsBS,subjectsFellow),1),PSE_Slope_Fellow(intersect(subjectsBS,subjectsFellow),1)) %paired samples
...
meanBSSlope = mean(PSE_Slope_BS(subjectsBS,2));
meanFellowSlope = mean(PSE_Slope_Fellow(subjectsFellow,2));


%medians
medianBSSlope = median(PSE_Slope_BS(subjectsBS,2));
medianFellowSlope = median(PSE_Slope_Fellow(subjectsFellow,2));

%group data for boxplot
Alldatatoplot = [PSE_Slope_BS(subjectsBS,2); PSE_Slope_Fellow(subjectsFellow,2)];
groups = [ones(1,length(subjectsBS))';2*ones(1,length(subjectsFellow))'];

individdata = {PSE_Slope_BS(subjectsBS,2), PSE_Slope_Fellow(subjectsFellow,2)};
...
disp('BS vs Fellow')
[p,h,stats] = ranksum(PSE_Slope_BS(subjectsBS,2),PSE_Slope_Fellow(subjectsFellow,2)) %independent samples
[p,h,stats] = signrank(PSE_Slope_BS(intersect(subjectsBS,subjectsFellow),2),PSE_Slope_Fellow(intersect(subjectsBS,subjectsFellow),2)) %paired samples
