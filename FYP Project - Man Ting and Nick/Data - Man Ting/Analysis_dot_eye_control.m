...
%% set parameters
ntrialseachcond = 10; %remember to double this number if testing con-vex and -cave together [convex =3]
...
for i = 4:5; %only fovea conditions
            ...
            for dotEye = 1:2
                if convex == 1 || convex == 2
                    ...
                    elseif i==3
                        tmp1 = find((subjectdata(:,2) == dotpos) & (subjectdata(:,1) == i) & (subjectdata(:,4) == convex) & subjectdata(:,5) == outsidekey)'; %OUTSIDE
                    elseif i==4
                        tmp1 = find((subjectdata(:,2) == dotpos) & (subjectdata(:,1) == i) & (subjectdata(:,3) == dotEye) & (subjectdata(:,4) == convex) & subjectdata(:,5) == outsidekey)'; %OUTSIDE
                    elseif i==5
                        tmp1 = find((subjectdata(:,2) == dotpos) & (subjectdata(:,1) == i) & (subjectdata(:,3) == dotEye) & (subjectdata(:,4) == convex) & subjectdata(:,5) == outsidekey)'; %OUTSIDE
                    end
...
%                     end
                ...
                %         conditions(j,:,i) = tmp;
                results(j,dotEye,i) = size(tmp1,2);
                %for the mistake in counterbalancing
...
figure; 
plot(1:nDotPos, results(1:nDotPos,1,4), 'ro-') %Occ Fovea BS eye
hold on;
plot(1:nDotPos, results(1:nDotPos,1,5), 'bs-') %Ctrl Fovea BS eye
plot(1:nDotPos, results(1:nDotPos,2,4), 'mo-') %Occ Fovea Fellow eye
plot(1:nDotPos, results(1:nDotPos,2,5), 'cs-') %Ctrl Fovea Fellow eye

axis([0.5 nDotPos+0.5 -0.5 ntrialseachcond+0.5])
[leg] = legend('Occ Fov BS', 'Ctrl Fov BS','Occ Fov Fellow', 'Ctrl Fov Fellow', 'Location', 'Northwest');
...
markerface{2} = 'None';
markerface{3} = 'None';
markerface{4,1} = colorpoints{4,1};
markerface{4,2} = colorpoints{4,2};
markerface{5,1} = colorpoints{5,1};
markerface{5,2} = colorpoints{5,2};
...
for condswitch = 4:5 %conditions
    ...
    for dotEye = 1:2
        ...
        NumPos = [results(1,dotEye,condswitch) results(2,dotEye,condswitch) results(3,dotEye,condswitch) results(4,dotEye,condswitch) results(5,dotEye,condswitch) results(6,dotEye,condswitch) results(7,dotEye,condswitch)];
        ...
        ProportionCorrectModel(condswitch,:, dotEye) = PF(paramsValues,StimLevelsFineGrain);
        
        
        plot(StimLevels,ProportionCorrectObserved,'LineStyle', 'None','Color', colorpoints{condswitch,dotEye},'Marker',markershape(condswitch,dotEye),'MarkerFaceColor', markerface{condswitch,dotEye},'markersize',10);
...
legend('Occ Fov BS', 'Occ Fov Fellow', 'Ctrl Fov BS', 'Ctrl Fov Fellow', 'Location', 'Southeast');

for condswitch = 4:5
    for dotEye = 1:2
        plot(StimLevelsFineGrain,ProportionCorrectModel(condswitch,:, dotEye),'-','color',colorpoints{condswitch,dotEye},'linewidth',2);
    end
end
